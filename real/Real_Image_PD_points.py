...
plot_image(Image_large, 'Real_ImageLarge')
...
for idx, deg in enumerate(degs):
    ...
    plot_variogram(OK,  (.0005,0.030), 'Real_variogram_orig_im')
    plt.show()
    plot_variogram(OK1, (.0005,0.030), 'Real_variogram_poly_%d' %deg)
    plt.show()
    plot_variogram(OK2, (.0005,0.030), 'Real_variogram_VSDK_%d' %deg)
    plt.show()
    plot_variogram(OK3, (.0005,0.030), 'Real_variogram_points_%d' %deg)
    plt.show()
    
    Outfile = open('Log_RealImage_%d.txt' %deg, "w")
    ...
    # Save data
    np.savetxt('Real_square_OriginalImage_xy', pts)
    np.savetxt('Real_square_OriginalImage_z', Image)
    np.savetxt('Real_ApproximatedImage_PPD_z_%d' %deg, Imageapprox)
    np.savetxt('Real_ApproximatedImage_PPD_VSDK_z_%d' %deg, Imageapprox1)
    np.savetxt('Real_square_OriginalImage_PPD_xy_%d' %deg, PDpts)
    np.savetxt('Real_square_OriginalImage_PPD_z_%d' %deg, fval)
    np.savetxt('Real_square_OriginalImage_PPD_xy_VSDK_%d' %deg, PDptsv)
    np.savetxt('Real_square_OriginalImage_PPD_z_VSDK_%d' %deg, fval)
    
    # Save images
    plot_image(Imageapprox1, 'Real_ApproximatedImage_PPD_VSDK_%d' %deg)
    plot_image(Imageapprox, 'Real_ApproximatedImage_PPD_%d' %deg)
    plot_image(Image, 'Real_OriginalImage')
    plot_image(Image, 'Real_OriginalImage_PPD_%d' %deg, pts=(xptsv,yptsv))

plot_error(CR_VSDK, MSE_VSDK,'CR','MSE','RealCR_VS_MSE_PPD_VSDK')
plot_error(CR_POLY, MSE_POLY,'CR','MSE','RealCR_VS_MSE_PPD')
plot_error(CR_VSDK, PSNR_VSDK,'CR','PSNR','RealCR_VS_PSNR_PPD_VSDK')
plot_error(CR_POLY, PSNR_POLY,'CR','PSNR','RealCR_VS_PSNR_PPD')
