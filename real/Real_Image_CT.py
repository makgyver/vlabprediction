...
matr = scipy.io.loadmat('sm_aprile.mat')
Image_large = matr["mat"]

plot_image(Image_large, 'Real_ImageLarge_CT')

sx, sy = Image_large.shape
lx = int(np.floor(0.0906*sx)-1)
rx = int(np.floor(0.208 *sx)-1)
ly = int(np.floor(0.473 *sy)-1)
ry = int(np.floor(0.51  *sy)-1)
...
vertex = np.array([[1,1,60,90,120,110], [m-1,90,90,90, 150,m-1]])
...
for idx, deg in enumerate(degs):
    ...
    # Compute the values at points
    fval = np.matrix([Image[int(np.floor(ypts[i]-1)),int(np.floor(xpts[i]-1))] for i in range(xpts.shape[0])]).T
    ...
    plot_variogram(OK,  (.002,0.031), 'Real_CT_variogram_orig_im')
    plot_variogram(OK1, (.002,0.031), 'Real_CT_variogram_poly_%d' %deg)
    plot_variogram(OK2, (.002,0.031), 'Real_CT_variogram_VSDK_%d' %deg)
    plot_variogram(OK3, (.002,0.031), 'Real_CT_variogram_points_%d' %deg)

    Outfile = open('Log_CT_RealImage_%d.txt' %deg, "w")
    ...
    # Save data
    np.savetxt('Raal_polyghon_OriginalImage_xy', bt)
    np.savetxt('Raal_polyghon_OriginalImage_z', Image)
    np.savetxt('Raal_polyghon_ApproximatedImage_TC_z_%d' %deg, Imageapprox)
    np.savetxt('Raal_polyghon_ApproximatedImage_TC_VSDK_z_%d' %deg, Imageapprox1)
    np.savetxt('Raal_polyghon_OriginalImage_TC_xy_%d' %deg, pts1)
    np.savetxt('Raal_polyghon_OriginalImage_TC_xy_VSDK_%d' %deg, wam1)
    ...
    # Save images
    plot_image(Imageapproxs1[lx-1:rx,ly-1:ry], 'Real_ApproximatedImage_CT_VSDK_%d' %deg, pts=(xpolygon-ly,ypolygon-lx))
    plot_image(Imageapproxs[lx-1:rx,ly-1:ry], 'Real_ApproximatedImage_CT_%d' %deg, pts=(xpolygon-ly,ypolygon-lx))
    plot_image(Image[lx-1:rx,ly-1:ry], 'Real_OriginalImage_CT_zoom', pts=(xpolygon-ly,ypolygon-lx))
    plot_image(Image, 'Real_OriginalImage_CT_%d' %deg, pts=(xpolygon,ypolygon),pts1=(xpts,ypts))
    
plot_error(CR_VSDK, MSE_VSDK,'CR','MSE','RealCR_VS_MSE_CT_VSDK')
plot_error(CR_POLY, MSE_POLY,'CR','MSE','RealCR_VS_MSE_CT')
plot_error(CR_VSDK, PSNR_VSDK,'CR','PSNR','RealCR_VS_PSNR_CT_VSDK')
plot_error(CR_POLY, PSNR_POLY,'CR','PSNR','RealCR_VS_PSNR_CT')
