...
sys.stdout = open("./log/svr_trvste_%s.log" %ttime, "w")
...
file_test, diff_te = (file_train, False) if not options["file_test"] else (options["file_test"], True)
look_back = options["look_back"]
test_size = options["test_size"]
future = options["future"]

#LOAD
#sequence = np.array(mat2list(file_train)[1])
sequence = np.load(file_train)
...
(X_tr, y_tr), (X_te, y_te) = create_dataset(sequence, train_size, look_back)

grid = {"C": [10**e for e in range(-1, 6)], "gamma": [10**e for e in range(-5,-1)], "epsilon": [10**e for e in range(-3,-1)]}
#grid = {"C": [1000], "gamma": [0.001], "epsilon": [0.01]}
print "Grid", grid
...
if diff_te:
	y_d = np.load(file_test)
	(_, _), (_, y_te) = create_dataset(y_d.reshape((y_d.shape[0],1)), train_size, look_back)
...
# shift train preictions for plotting
y_tr_pred_plot = np.empty_like(sequence).flatten()
y_tr_pred_plot[:] = np.nan
y_tr_pred_plot[look_back+1:len(y_tr_pred)+look_back+1] = y_tr_pred
# shift test predictions for plotting
y_te_pred_plot = np.empty_like(sequence).flatten()
y_te_pred_plot[:] = np.nan
y_te_pred_plot[len(y_tr_pred)+(look_back)+1:len(sequence)] = y_te_pred
# plot baseline and predictions
plt.plot(scaler.inverse_transform(sequence), label='actual data')
...
plt.xlabel('time')
plt.ylabel('value')
plt.title('SVR (RMSE:%.4f)' %te_score)
plt.legend()
plt.savefig('out/svr_full_plot_%s.eps' %ttime, format='eps', dpi=300)
plt.show()
plt.clf()
...
y_real_plot = scaler.inverse_transform(y_te)[-head-test_size:]
plt.axvline(x=head-.5, color='k', linestyle='--', linewidth=1)

#original = np.empty_like(y_tr_pred_plot).flatten()
#original[:] = np.array(mat2list("./datasets/original.mat"))[-head-test_size:]
if diff_te:
	plt.plot(y_d[-head-test_size:], label="reduced data")
plt.plot(y_real_plot, label='actual data')
#plt.plot(y_tr_pred_plot, label='training pred')
plt.plot(y_te_pred_plot, label='test prediction')
plt.xlabel('time')
plt.ylabel('value')
plt.title('SVR (RMSE:%.4f)' %te_score)
plt.legend()
plt.savefig('out/svr_zoom_plot_%s.eps' %ttime, format='eps', dpi=300)
plt.show()

arrays2file([np.array(range(y_real_plot.shape[0])), y_real_plot[:,0], y_te_pred_plot], "out/svr_zoom_plot_%s.txt" %ttime)
